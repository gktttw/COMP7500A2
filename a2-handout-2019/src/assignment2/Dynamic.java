...
public class Dynamic {
    ...
    private static int getCompromise(int i, int k, Job[] jobs) {
        int compromise = 0;
        for (int jobIndex = k + 1; jobIndex < i ; jobIndex++) {
            compromise += jobs[jobIndex].payment();
        }
    ...
    public static Solution optimalSolutionDynamic(int[] cost, int minShiftBreak,
        ...
        List<Job> jobsInShift = new ArrayList<>();
        while(i != jobs.length) {
            if (j == jobs.length) {
                ...
                if (jobs[i].compatible(jobs[k])) {
                    ...
                    if (jobs[i].end() - jobs[j].start() + 1 > maxShiftLength) {
                        ...
                        int take = storage[i+1][j][i];
                        int skip = storage[i+1][j][k] - jobs[i].payment();
                        int newShift = getProfit(cost, jobs, j, k ,i) + storage[nextJob][jobs.length][jobs.length] +
                                getCompromise(i, k, jobs);
        ...
        System.out.println(new Solution(shifts, chosenJobs));
